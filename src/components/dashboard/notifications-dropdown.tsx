...
export function NotificationsDropdown() {
  ...
  const { data: invoices, isLoading } =
    useCollection<RentalIncome>(activeInvoicesQuery);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative rounded-full h-9 w-9 border border-border/50"
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-80 mt-2 shadow-xl border-border/50"
      >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col">
              {invoices?.map((invoice) => {
                ...
                return (
                  <Link
                    href={`/dashboard/properties/${invoice.propertyId}?tab=income`}
                    key={invoice.id}
                  >
