      ...
      return (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <div
                className={`font-medium cursor-help ${isProfit ? "text-green-600" : "text-red-600"}`}
              >
                {formatCurrency(amount)}
              </div>
            </TooltipTrigger>
      ...
      const handleFinishConstruction = async () => {
        ...
        try {
          ...
          toast({
            ...
            description: `${property.name} is now a finished property.`,
          });
        } catch (error) {
          toast({
            variant: "destructive",
