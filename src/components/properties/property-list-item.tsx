...
import { LayoutGrid, Ruler, MapPin, Building2 } from 'lucide-react';
...
export function PropertyListItem({ property }: PropertyListItemProps) {
  ...
  return (
    <Card className="overflow-hidden hover:shadow-md transition-all duration-300 border-border/50 flex flex-col h-full">
      <CardHeader className="pb-3 space-y-3">
        ...
        <div>
          <CardTitle className="font-headline text-lg mb-1 line-clamp-1">
        ...
        {property.type === "Finished" ? (
            ...
            <div className='space-y-3'>
                ...
                <Progress 
                  ...
                  indicatorClassName={budgetProgress > 100 ? 'bg-destructive' : budgetProgress > 90 ? 'bg-amber-500' : 'bg-primary'} 
                  className="h-1.5" 
                />
                <div className="grid grid-cols-2 gap-2 text-[10px] pt-1">
                    <div>
                        <p className="text-muted-foreground uppercase tracking-wider font-semibold">Spent</p>
                        <p className="font-bold text-foreground">
                          {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'ZMW', maximumFractionDigits: 0 }).format(spent)}
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-muted-foreground uppercase tracking-wider font-semibold">Budget</p>
                        <p className="font-bold text-foreground">
