...
export default function AllPropertiesPage() {
  ...
  const { data: finishedProps, isLoading: isFinishedLoading } =
    useCollection<Property>(finishedPropsQuery);
  ...
  const { data: constructionProps, isLoading: isConstructionLoading } =
    useCollection<Property>(constructionPropsQuery);
  ...
  const { data: allExpenses, isLoading: isExpensesLoading } =
  ...
  const { data: allIncomes, isLoading: isIncomesLoading } =
    useCollection<RentalIncome>(incomesQuery);
  ...
  const { data: allMaintenance, isLoading: isMaintenanceLoading } =
  ...
  const portfolioNetProfit = calculatedProperties.reduce(
    (acc, p) => acc + (p.netProfit || 0),
    0,
  );
  const totalUnits = calculatedProperties.reduce(
    (acc, p) => acc + (p.units || p.unitsList?.length || 1),
    0,
  );

  const totalInvestment = calculatedProperties.reduce(
    (acc, p) => acc + (p.totalInvestment || 0),
    0,
  );
  const totalRent = calculatedProperties.reduce(
    (acc, p) => acc + (p.totalRentReceived || 0),
    0,
  );

  // Truncate portfolio ROI without rounding up
  const portfolioRoiRaw =
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Managing assets for {user?.email}
          </p>
        </div>
