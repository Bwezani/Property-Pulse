...
export default function ConstructionDashboardPage() {
  ...
  const { data: rawProperties, isLoading: isPropsLoading } =
  ...
  const { data: allExpenses, isLoading: isExpensesLoading } =
    useCollection<ConstructionExpense>(allExpensesQuery);
  ...
  const projectsByStage = constructionProperties.reduce(
    (acc, p) => {
  ...
  const budgetUtilizationRaw =
    totalPlannedBudget > 0
      ? (totalConstructionCost / totalPlannedBudget) * 100
      : 0;
    ...
    <div className="flex-1 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Development oversight for {user?.email}
          </p>
        </div>
        ...
        <KpiCard
          ...
          value={
            Object.keys(projectsByStage).length > 0
              ? Object.entries(projectsByStage).sort(
                  (a, b) => b[1] - a[1],
                )[0][0]
              : "N/A"
          }
      ...
      <div className="rounded-xl border bg-card p-6 shadow-sm space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Managing <strong>{totalActiveProjects}</strong> project(s).
          {totalPlannedBudget > 0 &&
            ` Budget utilization is ${budgetUtilization.toFixed(1)}%.`}
        </p>
