...
export default function ReportsPage() {
  ...
  const { data: finishedProps, isLoading: isPropsLoading } =
  ...
  const { data: rentalIncomes, isLoading: isIncomesLoading } =
  ...
  const { data: maintenanceExpenses, isLoading: isMaintenanceLoading } =
  ...
  const { data: constructionExpenses, isLoading: isConstructionLoading } =
    useCollection<ConstructionExpense>(constructionQuery);
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Portfolio performance analysis for {user?.email}
        </p>
      </div>
