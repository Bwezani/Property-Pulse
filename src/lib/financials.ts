...
export function calculatePropertyFinancials(
  ...
  const propertyConstructionExpenses = allConstructionExpenses.filter(
    (e) => e.propertyId === property.id,
  );
  const propertyRentalIncomes = allRentalIncomes.filter(
    (i) => i.propertyId === property.id && i.status === "Paid",
  );
  const propertyMaintenanceExpenses = allMaintenanceExpenses.filter(
    (e) => e.propertyId === property.id,
  );

  // Calculate total costs and incomes
  const totalConstructionCost = propertyConstructionExpenses.reduce(
    (sum, expense) => sum + expense.totalPrice,
    0,
  );
  const totalRentReceived = propertyRentalIncomes.reduce(
    (sum, income) => sum + income.amount,
    0,
  );
  const totalMaintenanceCost = propertyMaintenanceExpenses.reduce(
